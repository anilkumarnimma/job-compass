...
-- Drop the old overload (without expanded_terms parameter)
DROP FUNCTION IF EXISTS public.search_jobs(text, integer, integer, text);

-- Recreate the function with correct return type (real for rank)
...
RETURNS TABLE(
  ...
  posted_date timestamptz, created_at timestamptz, updated_at timestamptz,
  is_archived boolean, rank real
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    (CASE
      ...
      THEN 0.05::real
      WHEN NOT has_query THEN 0::real
      ELSE 0.01::real
    END) as rank
  FROM public.jobs j
  WHERE
